<x-app-layout bodyClass="{{ $bodyClass }}">
    <main>
        <form action="{{ route('mylab.experiment.presentiment.2.storeTrial') }}" name="ajax-info">
            @csrf
        ...
        <section class="part part-1">
            <h1>1. Experiment 2, Additional Control, PRNG</h1>
        </section>
        <section class="part part-2 hidden">
            ...
            <form action="{{ route('mylab.experiment.presentiment.2.searchUser') }}" method="post" name="ajax-search-user">
                @csrf
                <label>
